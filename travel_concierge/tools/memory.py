...
def _set_initial_states(source: Dict[str, Any], target: State | dict[str, Any]):
    ...
    if constants.ITIN_INITIALIZED not in target:
        ...
        # Ensure all required context variables exist with default values
        # This prevents template processing errors
        target[constants.ITIN_START_DATE] = "TBD"
        target[constants.ITIN_END_DATE] = "TBD"
        target[constants.ITIN_DATETIME] = "TBD"

        # Initialize common template variables for sub-agents
        target["origin"] = ""
        target["destination"] = ""
        target["start_date"] = ""
        target["end_date"] = ""
        target["outbound_flight_selection"] = ""
        target["outbound_seat_number"] = ""
        target["return_flight_selection"] = ""
        target["return_seat_number"] = ""
        target["hotel_selection"] = ""
        target["room_selection"] = ""
        target["poi"] = ""
        target["flight"] = ""
        target["hotel"] = ""

        # Ensure user_profile exists
        if constants.PROF_KEY not in target:
            target[constants.PROF_KEY] = {}

        # Update with source data
        target.update(source)

        # Set up itinerary dates if available
        itinerary = source.get(constants.ITIN_KEY, {})
        if itinerary and itinerary.get(constants.START_DATE) and itinerary.get(constants.END_DATE):
        ...
        print(f"Initialized context variables:")
        print(f"  {constants.ITIN_START_DATE}: {target.get(constants.ITIN_START_DATE)}")
        print(f"  {constants.ITIN_END_DATE}: {target.get(constants.ITIN_END_DATE)}")
        print(f"  {constants.ITIN_DATETIME}: {target.get(constants.ITIN_DATETIME)}")
        print(f"  {constants.PROF_KEY}: {target.get(constants.PROF_KEY)}")
        print(f"  origin: {target.get('origin')}")
        print(f"  destination: {target.get('destination')}")
